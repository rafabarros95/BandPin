"""
BandPin — shared constants
--------------------------
Single source of truth for UUIDs, the digit layout, and the BLE event
protocol. Must stay in sync with:
  Backend/BandPinFirmware/BandPinFirmware.ino   (ESP32, Arduino C++)
  Frontend/.../ble/BandBleClient.kt             (Wear OS app, Kotlin)
"""

# ── BLE GATT UUIDs ───────────────────────────────────────────────────────────
BANDPIN_SERVICE_UUID = "4A420001-1000-8000-0080-00805F9B34FB"
EVENT_CHARACTERISTIC_UUID = "4A420002-1000-8000-0080-00805F9B34FB"
DEVICE_NAME = "BandPin"

# ── Digit layout (fixed, per the BandPin proposal) ───────────────────────────
# Strip A (top of wrist):  digits 0–4, zone 0 at the strip start
# Strip B (palm side):     digits 5–9, zone 0 at the strip start
NUM_STRIPS = 2
ZONES_PER_STRIP = 5
PIN_LENGTH = 4

# ── Wiring ───────────────────────────────────────────────────────────────────
# Both Trill Flex boards keep the factory address 0x48; each strip sits on its
# own ESP32 I²C controller, so there is no address conflict and no breadboard
# or Y-cable is needed inside the band.
#
#   strip A → TwoWire(1), SDA GPIO 25 / SCL GPIO 26
#   strip B → Wire,       SDA GPIO 21 / SCL GPIO 22
#   common GND
#
# Both strips are optional at boot and are re-probed once per second while
# running, so either can be unplugged and reconnected without a restart.
TRILL_ADDR_STRIP_A = 0x48
TRILL_ADDR_STRIP_B = 0x48

STRIP_A_PINS = (25, 26)   # (SDA, SCL)
STRIP_B_PINS = (21, 22)   # (SDA, SCL)

I2C_FREQUENCY = 400_000

TRILL_PRESCALER = 3
TRILL_NOISE_THRESHOLD = 200

# ── Active region of the strip ───────────────────────────────────────────────
# Only the mounted part of each Trill Flex is reachable on the band. Touches
# outside this window are discarded; positions inside it are rescaled to
# 0.0–1.0 before being split into zones. Retune after remounting the strips.
ACTIVE_START = 0.08
ACTIVE_END = 0.39

# ── Gesture parameters (mirror of the firmware; pilot-test these) ────────────
TAP_MAX_MS = 250              # max contact time to count as a tap
DOUBLE_TAP_GAP_MS = 400       # max pause between the two taps of a double-tap
HOLD_DELETE_MS = 2000         # hold this long without sliding = delete
HOLD_REPEAT_DELETE_MS = 600   # keep holding = delete again, every …
ZONE_HYSTERESIS = 0.04        # fraction of strip length, anti-flicker margin
SAMPLE_INTERVAL_MS = 10       # 100 Hz sensor loop

# ── BLE event payload (6 bytes) ──────────────────────────────────────────────
#   [0] event type (see EVENT_* below)
#   [1] strip index      (0 = A, 1 = B)
#   [2] digit            (0–9, already offset by the firmware)
#   [3] position * 100   (0–100, within the active region)
#   [4] board millis() >> 8   (low 16 bits of ESP32 clock, for latency analysis)
#   [5] board millis() & 0xFF
BLE_PAYLOAD_SIZE = 6

EVENT_DOWN = 0     # finger lands on a strip
EVENT_TICK = 1     # zone boundary crossed while sliding → haptic tick
EVENT_UP = 2       # finger lifted
EVENT_SELECT = 3   # double-tap → digit chosen
EVENT_DELETE = 4   # hold ≥ HOLD_DELETE_MS → remove last digit

EVENT_NAMES = {
    EVENT_DOWN: "DOWN",
    EVENT_TICK: "TICK",
    EVENT_UP: "UP",
    EVENT_SELECT: "SELECT",
    EVENT_DELETE: "DELETE",
}
